...
# Function to apply random rotation, scaling, noise, Gaussian blur, color jitter, and translation to the image
def apply_augmentations(image):
    ...
    image = F.resize(image, (int(image.size[0] * scale_factor), int(image.size[1] * scale_factor)))

    # Random noise
    noise = np.random.normal(loc=0, scale=10, size=image.size)
    noise_image = Image.fromarray(np.uint8(noise))
    image = Image.blend(image, noise_image, alpha=0.1)

    # Random Gaussian blur (kernel size 3x3 to 5x5)
    ...
    image = F.adjust_saturation(image, random.uniform(0.5, 1.5))
    image = F.adjust_hue(image, random.uniform(-0.1, 0.1))

    # Random translation (-10 to 10 pixels horizontally and vertically)
    x_translate = random.randint(-10, 10)
    y_translate = random.randint(-10, 10)
    image = F.affine(image, angle=0, translate=(x_translate, y_translate), scale=1, shear=0)

    return image

# Modified capture_photos function with augmentation
def capture_photos(video_capture, name, roll_number, parent_folder, embeddings_df, num_photos=15, interval=3):
    ...
    for i in range(num_photos):
        ...
        if ret:
            ...
            # Apply augmentations
            pil_image = apply_augmentations(pil_image)
